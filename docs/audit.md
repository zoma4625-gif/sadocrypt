...
### 2. パズルIDの衝突（32bit ＋ 上書きノーチェック）🔴
**症状**: `uuidv4().slice(0,8)` ＝ 8 hex ＝ 32bit（約43億通り）。誕生日問題で**約6.5万リンクで初回衝突が現実圏**。さらに `env.PUZZLES.put(id, …)` が存在チェック無しのため、衝突時に**別人のパズルを黙って上書き**（データ消失・誤配信）。利用が伸びるほど確率上昇、サイレントに起きる。

**直す方向**: IDを12〜16 hex（48〜64bit）に伸ばす。または `put` 前に `get` で存在確認し衝突時はリトライ採番。両方やると堅い。
