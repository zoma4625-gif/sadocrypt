...
| 日付 | 変更内容 |
|------|----------|
| 2026-07-12 | fix(howto): 使い方トグル押下時にショーケース（スマホ版カバーフローカルーセル）の自動再生が復帰しない不具合を修正（`encrypt.js`）: `initCflow()`の自動再生（`cfAutoTimer`）は「手動操作（スワイプ・ドット・矢印クリック）で完全停止・再開しない」設計だが、`switchHowto()`側は旧実装だと「受け取る人パネルを初めて表示した時だけ」先頭にリセットして止める処理しかなく、一度手動操作で自動再生が止まると以降トグルを押しても復帰しなかった。`initCflow()`に`wrap._cfRestartAuto()`（先頭にリセットして`cfAutoTimer`を再設置）を追加し、`switchHowto()`をパネル種別に関わらず毎回呼び出す形に変更。送る人・受け取る人どちらの切替でも、トグルを押すたびにそのパネルのショーケースが先頭から自動再生し直す。Playwright(Chromium・モバイル幅390px)で「自動再生→手動操作で停止→トグル押下で先頭に戻り自動再生再開」を実測確認済み。 |
| 2026-07-12 | feat(decrypt): LINE等アプリ内ブラウザ(WebView)でも復号後ファイルを保存できるよう3段フォールバックを実装（`src/pages/decrypt.js`）: 背景＝WebViewは`<a download>`のBlob URL保存に対応しておらず「外部アプリケーションを開こうとしています」ダイアログが出て許可/不許可どちらでも保存されなかった。**実装**: ①`isInAppWebView()`（UA内に`Line/`・`FBAN`・`FBAV`・`Instagram`・`Twitter`・`MicroMessenger`のいずれかを含むかで判定。**正規表現は使わず`indexOf`の部分一致にした**——このファイルはJSテンプレートリテラル内に埋め込まれておりバックスラッシュが外側のテンプレートで消費されてしまうため（`\b`→バックスペース制御文字に、`\/`→ただの`/`になり閉じ忘れと同じ扱いになる)、`/\bLine\//`のような正規表現を書くと構文エラーになる。実際にこれで一度ハマり、`node --check`はテンプレート内の文字列までは検証しないため気づきにくい。同種のクライアントJSを今後このファイルに追記する際は正規表現の中のバックスラッシュに注意すること）。②`canShareFile(file)`で`navigator.canShare({files:[file]})`をフィーチャー検出。③`dlLink()`を拡張: 通常ブラウザ（`!isInAppWebView()`）は従来どおり`<a download>`のまま完全に無変更。WebViewの場合は`<button class="dl-share-btn" data-dlid="N">`に切り替え、実体のBlobは`_dlBlobs[]`配列に保持（data属性にはindexのみ）。④WebView かつ share 不可と判定できた時点で`.letter-webview-guide`（「このアプリ内のブラウザでは保存できません。メニューから外部ブラウザ（Safari/Chrome）で開くと保存できます。」EN/JA対応）をボタン直下に**事前表示**。画像の場合のみ「画像は長押しでも保存できます。」を追記。⑤`.dl-share-btn`のクリックは`card`への委譲リスナーで処理: `canShareFile()`再判定→OKなら`navigator.share({files:[new File([blob],fname,{type:mime})]})`を実行（`AbortError`＝ユーザーがシートをキャンセルしただけなので何もしない）。share自体が使えない/失敗した場合はガイドをその場に動的挿入（まだ無ければ）。⑥「その他ファイル」形式の自動ダウンロード（`setTimeout`→`a.click()`）はWebViewでは実行しない（ユーザー操作なしに外部アプリ確認ダイアログが出てしまうため）。**検証**: `wrangler dev`+Playwright(Chromium)でUAスプーフィング・`navigator.share`/`canShare`のモック/削除により3パターン（通常ブラウザ／WebView+share可／WebView+share不可）を再現し、それぞれ期待どおりの挙動（`<a download>`維持／`share()`が正しいFile名・MIMEで呼ばれる／ガイド文言が画像有無で正しく出し分けられる、JA/EN両方）を確認済み。実機（iPhone LINE内ブラウザ）での最終確認は未実施のため別途お願いしたい。 |
| 2026-07-12 | fix(decrypt): 裏タブでの復号計算ほぼ停止バグを修正（`public/decrypt-worker.js` + `src/pages/decrypt.js`）: 原因＝`decrypt-worker.js` の計算ループが「50ms計算 → `setTimeout(chunk,0)` で次チャンクへ継続」という**setTimeout駆動**の構造だったため、タブを裏に回すとブラウザのバックグラウンドタイマースロットリング（Chrome: 背景タブのタイマーを最短1000msにクランプ、5分以上背景だと約1分に1回まで間引く「Intensive Throttling」／Safari・iOSも背景タブのタイマーを大幅に間引く）の対象になり、実質1/20以下（クランプ後は理論上さらに悪化）の速度に低下していた。メインスレッド側（進捗表示150ms・checkpoint保存1s・シーン演出）はWorkerの`postMessage`を受けて描画するだけの受動的な構造で計算の継続には無関係だったため、フリーズの主因ではないことをコード読解で確認済み。**修正**: ①`decrypt-worker.js` の `chunk()`（setTimeoutで自己再帰）を `run()`（`_i<_total`の間ノンストップで回り切る単一while文）に全面書き換え。setTimeoutを一切使わず、`postMessage`（タイマーではなくメッセージキューへの即時投函でスロットリング対象外）のみで progress/checkpoint を間引き送信する自走ループに変更。Workerスレッドを長時間ブロックしてもメインスレッド（UI）の応答性は損なわれない（別スレッドのため「ページが応答していません」の対象にもならない）。②`decrypt.js` のWorker不可時フォールバック（メインスレッド計算）は、UI応答性のため引き続き定期的なイベントループ譲渡が必要なため、`setTimeout(r,0)` を `MessageChannel` ベースの `yieldToEventLoop()` に置き換え（setTimeout系の背景タブクランプの対象外だが、rAF等ほかのメインスレッド系抑制の影響が完全にゼロになるわけではない旨をコメントに明記。Worker自走ループが可能な現代ブラウザではこのフォールバックはほぼ通らない想定）。iOSのページ完全サスペンド（OS都合で本当に実行が止まるケース）はこの修正の対象外とし、既存のresume機構（`RESUME_KEY`・checkpoint保存）で復帰後に続きから計算する前提を`decrypt-worker.js`冒頭コメントに明記。**検証**: `wrangler dev` + Playwright(Chromium)で実パズルを生成しWorker経路・フォールバック経路の両方で正しく完了・復号されることを確認（`bgtest-...`のテストメッセージが正しく復号表示された）。裏タブ実測A/B比較は、Playwright/PuppeteerがCDP（DevToolsプロトコル）接続下のページに対してブラウザ側が背景タブスロットリングを無効化する仕様のため自動化環境では再現不能と判断（`document.visibilityState`を複数タブのフォーカス移動・ウィンドウ最小化・完全重畳・関連Chromiumフラグの無効化除去など5パターンで検証したが一貫して`visible`のままだった。これはPlaywright自体の既知の制約であり本バグの調査結果を弱めるものではない）。原因はChrome/Safari共通の公知のタイマースロットリング仕様とコード構造から一意に特定できている。 |
| 2026-07-12 | style(bg): コーラル/セージの追従背景ブロブ（`position:fixed` の玉ボケ2個）をLP（`encrypt.js`）以外の全ページから完全削除（`philosophy.js`・`time-lock.js`・`terms.js`・`privacy.js`・`about.js`）。2026-07-10 に追加したモバイル限定 `@media(max-width:680px){.bg-blob{display:none}}` 分岐とPC/モバイル二重管理になっていたため、CSSルールごと削除し `<div class="bg-blob" ...>` 要素2個も各ページから撤去（PC/モバイル問わず「LPのみ表示」に一本化）。復号ページ・404・500（`_LIGHT_CSS`・`buildExpiredHtml`）・`/benchmark` はもともとこの装飾を持たないため対象外（確認のみ実施）。ブロブに依存するレイアウト（`.legal-wrap` 等の `z-index`）は無変更でも崩れないことを確認。 |
| 2026-07-12 | style(text): 「Brake.とは」カード1の説明文を「閲覧の難易度を上げ、」→「閲覧の敷居を上げ、」に修正（`i18n.js` の `about.card1.desc` ja値 + 未使用の `about.js` 内ハードコード）。英訳（`about.card1.desc` en値）は意味が変わらないため無変更。 |
| 2026-07-10 | style(bg): モバイル幅（`max-width:680px`）でコーラル/セージの追従背景ブロブ（`position:fixed` の玉ボケ2個）を非表示化（`philosophy.js`・`time-lock.js`・`terms.js`・`privacy.js`・`about.js`）。各ページの2個の `<div style="position:fixed;...">` に `class="bg-blob"` を付与し、`${HEADER_CSS}` 直後に `@media(max-width:680px){.bg-blob{display:none}}` を追加。PC表示・LP（`encrypt.js`。この背景ブロブ自体を持たないため対象外）は無変更。 |
| 2026-07-10 | fix(i18n): EN版で未配線だった日本語ハードコード箇所を全面配線（`i18n.js` + `encrypt.js` + `decrypt.js` + `handlers.js` + `router.js` + `terms.js` + `privacy.js`）: ①`i18n.js`: 既存だが未使用だった `about.*`/`howto.*`/`scene.name.*` を確定訳で上書き・新規キー追加（`howto.alt.*`・`howto.carousel.*`・`form.err.*`・`form.file.compress.msg1/2`・`enc.popup.done`・`scene.modal.title`・`dec.error.label`・`terms.title`/`terms.h1`・`privacy.title`/`privacy.h1`・`api.err.*`）。`dec.warn.long`/`dec.warn.long2` の en 訳を確定文言に更新。②`encrypt.js`: 「Brake.とは」「使い方」セクション全文・画像alt（12箇所）・カルーセルaria-label・シーン選択モーダル（タイトル+タイル12種+TILE_NAMES）・暗号化中ポップアップの状態文言・＋ボタンのaria-label/tooltip・バリデーション/実行時エラー文言（9箇所）を `T('key', lang)` に配線。JS文字列リテラルへの埋め込みは英訳にアポストロフィを含む箇所（Web Worker非対応・ファイル読込失敗）のみ `JSON.stringify(T(...))` でエスケープ。③`decrypt.js`: `dec-em` エラーラベル・1時間超警告（時間/日/ヶ月の単位語含む）・`opensAtText`（`Intl.DateTimeFormat` で英語は "Opens on Jul 3, 2026 at 15:12" 形式）・残り時間表示（`Xh/Xm/Xs left`）を `_l`（`window._BRAKE_LANG`）分岐で追加。④`handlers.js`: `handleSave(request, env, lang)` に `lang` 引数を追加し全エラーメッセージ（リクエスト過大・パラメータ不足・各フィールド不正・上限超過・保存失敗）を `T()` 化。⑤`router.js`: `handleSave` 呼び出しに `lang` を追加、`_LIGHT_CSS` を定数から `(lang) => ...` 関数に変更し404/500ページの `<html lang>` 属性を実際の言語に追従させた。⑥`terms.js`/`privacy.js`: `<title>`/`<h1>` を `T()` 化（本文は意図的に日本語のまま・変更なし）。シーン12種の英訳（`scene.name.*`）は既存DICT値（Digits/Panels/Auto/Pulse/Orbit/Ripple 含む）をユーザー確認の上でそのまま採用。`wrangler dev` + `scripts/synthetic-monitor.mjs` で実パズルを生成しJA/EN両方の `/`・`/:id`・`/terms`・`/privacy`・`/api/save` エラー応答・404ページを確認済み。 |
| 2026-07-10 | feat(i18n): EN/JA 多言語対応を全実装（`src/i18n.js` 新規 + router.js / header.js / footer.js / handlers.js / encrypt.js / decrypt.js / terms.js / privacy.js / public/scenes.js）: ①`src/i18n.js` を新規作成: `T(key, lang)` 翻訳ルックアップ・`getLang(request)` (cookie `brake_lang=ja|en` → Accept-Language フォールバック → 'ja')・`formatDate(date, lang)` (Intl.DateTimeFormat)・`formatRemaining(sec, lang)`・`LANG_SWITCH_JS(lang)` (クライアントに `window._BRAKE_LANG` を注入 + cookie切替ハンドラ) を実装。②全ページ export を `const HTML_XXX = \`...\`` → `function HTML_XXX(lang) { return \`...\` }` 形式に変換。③router.js で `const lang = getLang(request)` を取得し全ページ呼び出しに渡す。④header.js / footer.js を `HEADER_HTML(lang)` / `HEADER_JS(lang)` / `FOOTER(lang)` 関数化し EN/JA 切替ボタンをナビに追加。⑤handlers.js: `buildExpiredHtml(lang)` / `handleSharedPuzzle(..., lang)` に lang 引数追加、循環インポート回避のため `HTML_DECRYPT` を関数引数として受取り `htmlDecrypt(lang).replace(...)` で呼び出す。⑥terms.js / privacy.js: EN モード時に「The Japanese text is the governing version. English translation is in preparation.」バナーを表示（法的本文は日本語のまま）。⑦encrypt.js / decrypt.js: LP 全UI文言を `T('key', lang)` に、decrypt.js のクライアント動的文言は `window._BRAKE_LANG` を `var _l` に代入して三項演算子で分岐。⑧scenes.js: MSGS 配列に `te`（英語）フィールドを追加、`msgText(m)` / `L(ja, en)` ヘルパーで全 showFin・flow hint・dawn rotator をバイリンガル化。 |
| 2026-07-10 | feat(monitor): 合成監視（synthetic monitoring）を追加（`orison-worker/scripts/synthetic-monitor.mjs` + `.github/workflows/synthetic-monitor.yml`）: 1日1回（00:00 UTC = 09:00 JST）、encrypt→save→fetch→solve→decrypt の一周を Node.js ESM スクリプトで自動実行。失敗時は GitHub Actions がエラーを通知。外部依存ゼロ（Node.js 18+ 組み込みのみ）。cc=50000（~55ms）で高速テスト。`workflow_dispatch` で手動実行も可能。スクリプト単体実行: `node orison-worker/scripts/synthetic-monitor.mjs [baseUrl]`。 |
...
| 2026-07-09 | fix(sw): 画像暗号化「Load failed」再発を修正（public/sw.js）: 原因は2層。①旧SW（catch-all付き）が iOS Safari に残存し大きい POST を respondWith で再フェッチして失敗（テキスト＝小サイズは通る・画像＝大サイズは失敗、という差と一致）。②`navigate` ハンドラ（`event.respondWith(fetch(req,{cache:'no-store'}))`）が iOS Safari では JS `fetch()` の大きい POST を mode:'navigate' として届けるケースがあり respondWith を誤発火させていた可能性。修正: navigate ハンドラを完全削除（Workers が Cache-Control:no-cache を返すため SW 介入不要）。CACHE_NAME を `brake-pwa-v1` → `brake-pwa-v2` に上げ旧SW強制更新・旧キャッシュ削除。SW が respondWith を呼ぶのは share_target POST（pathname==='/' && method==='POST'）のみに厳限化。 |
| 2026-07-09 | style(result): テキストカード「via Brake.」上の隙間を詰める（decrypt.js）: コピーボタン（height ~29px）が `.letter-foot` の行ボックスを押し広げ、日時テキスト（10px）が行中央に配置されるため「行の下端〜via Brake.」の実距離が ~13px になっていた。テキストカード専用クラス `.letter-foot--text { align-items:flex-end }` を追加し日時テキスト下端を行ボックス底に揃えた。`.letter-foot--text .letter-btn2 { padding:4px 12px }` でコピーボタン縦パディングを 6px→4px に縮小。これにより視覚的ギャップは `.letter-via-inner` の `margin-top:4px` のみになる（日時テキスト下端〜via 上端 ≈ 4-6px）。ファイルカード（`.letter-sig`）は無変更。 |
| 2026-07-09 | fix(sw): reg.update() を削除し [4/送信] 遅延・Load failed を修正（encrypt.js）: 根本原因: `reg.update()` を毎ページロード時に呼ぶと sw.js の更新チェック用ネットワークリクエストが発生する。これが `/api/save` POST と同時に走ると iOS/Mac Safari では2リクエストが競合し [4/送信] が5秒遅延またはLoad failed になっていた。ブラウザはナビゲーション時に SW 更新チェックを自動実行するため `reg.update()` は不要。SW コード自体（v2・catch-all削除）は変更なし。 |
| 2026-07-09 | fix(encrypt): Blob体変更を差し戻し＋5段階ラベル付きログを追加（encrypt.js）: 前回の Blob body 化（Mac Safari 対策）が iOS Safari でも「[4/送信] Load failed」を引き起こし両方壊れた。調査: Mac Safari の「Load failed」は旧SW catch-all（既修正）が原因であり、文字列 body 自体は問題なかった可能性が高い。対処: fetch body を `new Blob([...])` → `JSON.stringify(saveBody)` + 明示 `Content-Type: application/json` ヘッダーに戻す（SW 修正後は元コードで動くはず）。同時に [1/読込][2/暗号化][3/構築][4/送信][5/応答] の5段階に try/catch を分割し、各段階の開始・完了を `console.log`、失敗時にエラーメッセージに段階ラベルを付与（例: 「[4/送信] Load failed」）。bodySize・ct.length 等の診断情報もコンソールに出力。 |
| 2026-07-09 | style(result): テキストカードの「via Brake.」上下余白を詰める（decrypt.js）: `.letter-via-inner{margin-top:6px}` → `margin-top:4px` に縮小（開封記録行との間隔を詰め「続きの一行」に見えるよう調整）。`.letter-card` の `padding-bottom:18px` → `12px` に変更し via とカード下端の余白も詰める。 |
| 2026-07-09 | fix(result): テキストカードの「via Brake.」位置修正（decrypt.js）: カード外右側にはみ出していた問題を修正。`showViaExt()`（カード外DOM挿入）をやめ、テキストカードの `inner` HTML に `.letter-via-inner`（`margin-top:6px`）div を直接追加してカード内・左下に収める。ファイルカード（`.letter-sig` 右端）・URL・YouTube の配置は変更なし。 |
...
| 2026-07-08 | fix(legal): privacy.js / terms.js を実装と整合させる（4箇所修正・コア変更なし）: ①privacy.js 第2条1項: Referrer-Policy 表記を `no-referrer` → `strict-origin-when-cross-origin` に修正し説明文を実装に合わせた。②privacy.js 第2条に「2-3. 共有機能で受け取ったデータの扱い」を新設（URL即時消去・サーバー送信はユーザー実行時のみ）。③privacy.js 第6条: `sadocrypt_cache_[ID]` の削除条件に「復号に失敗した場合」を追加。`brake_resume_[ID]`（復号再開データ）の項目を新規追加（保存内容・タイミング・完了時自動削除を記載）。④terms.js 第5条 item3「データの自動削除」に有効期限の具体値（設定した復号時間＋1か月）と「復号後も有効期限まで再アクセス可能」の一文を追加（復号後即削除の誤解防止）。 |
| 2026-07-08 | audit(legal): データライフサイクルと規約/PP の突き合わせ調査（コード変更なし）: 発見されたギャップは4点。①RESUME_KEY（`brake_resume_{id}`）が privacy.js 第6条に未記載・有効期限フィールドなし（重大）。②privacy.js 第2条1項の `Referrer-Policy: no-referrer` 記載が実装（security.js: `strict-origin-when-cross-origin`、2026-07-06 変更）と不一致（重大）。③完了キャッシュ削除トリガーが privacy.js 記載（有効期限超過・手動削除）のほかに AES-GCM 復号失敗時削除が存在（軽微）。④SW/share_target インメモリ一時保持（揮発性）が法的文書に未記載（軽微）。次のアクション: terms/privacy の書き換えが必要になった段階で①②を優先修正。 |
| 2026-07-08 | fix(sw): navigation リクエストに `{cache: 'no-store'}` を追加（public/sw.js）: SW の fetch() がブラウザ HTTP キャッシュを経由するため、ハードリフレッシュしても古い HTML が返り続ける問題を修正。`req.mode === 'navigate'` を検出し `fetch(req, {cache: 'no-store'})` でバイパス。 |
| 2026-07-08 | fix(cache): HTML ページの Cache-Control を `public, max-age=3600` → `no-cache` に変更（router.js）: デプロイ後もブラウザが旧 HTML を最大1時間キャッシュし続け、新コードが実機で反映されない問題を修正。Cloudflare Workers は動的レスポンス（ETag なし）のため `no-cache` = 実質毎回フェッチ。sitemap.xml / robots.txt（静的）は `max-age=86400` 維持。 |
| 2026-07-08 | refactor(form): カスタム時間入力を SSOT 型に全面刷新（encrypt.js）: `currentValue`（数値）+ `currentUnit`（単位）を唯一の真の値として導入。①FP誤差なし 2桁切り上げ `ceil2(v) = Math.ceil(Number((v*100).toFixed(4)))/100` を実装（例: 1.5555→1.56、1.55→1.55 ← FP誤差で1.56にならない）。②`renderFromTruth(writeBack)`: SSOT → liveEl/slider/chips/tvEl/tuEl を一括再描画。③`renderPreview()`: 手入力中に tvEl を書き換えずにプレビューのみ更新。④`confirmInput()`: blur/Enter 時に入力を正規化→ceil2→SSOT 更新→renderFromTruth(true) で全同期。⑤各ハンドラを SSOT read/write のみに整理: チップクリック・スライダー操作は currentValue/currentUnit を直接更新→renderFromTruth(true)、tvEl.input は renderPreview() のみ、tuEl.change は currentUnit 更新→renderFromTruth(true)（確定済み値を保持・手入力中の値を上書き）、blur/Enter は confirmInput()。⑥doEncrypt の二重丸め（`parseFloat(rawV.toFixed(2))`）を削除（tvEl.value が confirmInput 済み）、`isNaN` を `isFinite` に変更。解消したズレ: ①入力「1.5555」確定後、入力欄・見出し・真の値がすべて「1.56」で一致（ズレA）。②confirm で整数に戻らない（ズレB）。③スライダー未操作なら小数が保持される（ズレC）。 |
| 2026-07-07 | feat(share): スマホのハンバーガーメニューに「共有に追加」導線カードを実装（shared/header.js）: ①CSS: `.mob-share-wrap`（flex中央揃え・padding）/ `.mob-share-card`（position:relative・max-width:240px）/ `.mob-share-paper1/2`（コーラル#f7ddcf・青#e6ebf0、translate でずらし）/ `.mob-share-inner`（生成り地・0.5px枠・16px角丸）/ `.mob-share-desc`・`.mob-share-btn`（accent-grad・20px角丸ピル）を追加。②HTML: `.mobile-menu-links` と `.mobile-menu-footer` の間に `.mob-share-wrap` ＋ `.mob-share-card` DOM を挿入。③JS: `HEADER_JS` 先頭に `beforeinstallprompt` 捕捉（`_hdrInstallPrompt`・全ページ共通）。カード初期化IIFE: standalone判定→`.mob-share-wrap` を `display:none` / iOS→`IOS_SHORTCUT_URL` を `window.open` / Android/PC→`_hdrInstallPrompt.prompt()` 発火→承認時に非表示。 |
| 2026-07-07 | fix(share): IOS_SHORTCUT_URL を確定リンクに差し込み（encrypt.js）: `IOS_SHORTCUT_URL = 'https://www.icloud.com/shortcuts/7aaaa3cbc6b24fd5a421e23cc27edc44'` を設定。iOS での「共有に追加」ボタンが「準備中…」表示ではなくショートカット追加画面へ直接遷移するようになった。 |
| 2026-07-07 | fix(form): doEncrypt の秒数換算を toFixed(2) に統一（encrypt.js）: `doEncrypt()` 内の `Math.ceil(rawV * 100) / 100` を `parseFloat(rawV.toFixed(2))` に変更（blur ハンドラ・ceilTv と統一）。FP 誤差で 1.55 → 1.56 に切り上がり、最終秒数が 5580 → 5616（+36秒）になるケースを修正。 |
| 2026-07-07 | fix(form): カスタム時間 Enter 確定で整数に戻るバグを修正（encrypt.js）: ①blur ハンドラ・`ceilTv()` の `Math.ceil(raw * 100) / 100` を `parseFloat(raw.toFixed(2))` に変更。JS浮動小数点誤差で `2.55 * 100 = 255.0000…03` → `Math.ceil` → 256 → 2.56 に切り上がる問題を解消し、2.55 → 2.55 のまま保持。②keydown Enter ハンドラで `tuEl.focus()` 前に `saved = tvEl.value` を保存し、focus 後に値が変わっていた場合に復元する防衛コードを追加（ブラウザが Enter を select に処理して値をリセットするケースへの対応）。 |
| 2026-07-07 | fix(pwa): PWAアイコンの白背景角はみ出しを修正（public/icon-192.png・icon-512.png・manifest.json）: アイコンを「#fdfbf5 フルブリード背景＋四畳半ロゴ75%縮小中央配置（192:24px/512:64px パディング）」で再生成（qlmanage SVG→PNG）。`manifest.json` の icons エントリを `any` と `maskable` に分離（512.pngを2エントリ）。OSの角丸マスクで白カドが出なくなる。 |
| 2026-07-07 | fix(form): 送信ボタン案内の位置バグを修正（encrypt.js）: `position:absolute` 方式が `.form-run-wrap` 全体下端（`.form-run-note` の下）を基準にし案内が最下部に出ていた。`max-height:0→2em; overflow:hidden; padding-top:0→8px; opacity:0→1` トランジション方式に変更し、ボタンと `.form-run-note` の間に正しく表示。非表示時は高さゼロでスペースなし。 |
| 2026-07-07 | feat(form): 送信ボタン活性化フィードバック（encrypt.js）: `#run-hint`（opacity フェード・薄インク色 rgba(60,55,48,.55)・12px）をボタン下に追加。非活性状態（`.btn-run--off`）でクリック時に `.visible` クラスを付与して「メッセージかURLを入力してください」を静かにフェードイン。入力イベントで即非表示。ボタン自体の見た目（薄コーラル・cursor:not-allowed）は変更なし。 |
| 2026-07-07 | fix(share): 共有導線カードの表示条件修正（encrypt.js + shared/header.js）: macOS を `isMac()`（userAgentData.platform / navigator.platform + ontouchend 判定）で検出し PC右余白カード・モバイルメニューカードとも非表示。iOS は従来通り表示。Android/Windows Chrome 等は `beforeinstallprompt` 発火時のみ表示する遅延方式に変更（イベントが来なければ非表示のまま）。CSS の `@media(min-width:880px){ .share-card{display:block} }` 自動表示を削除し JS で制御に一本化。 |
| 2026-07-07 | fix(share): 共有導線カード説明文の改行位置修正（encrypt.js + shared/header.js）: 「他のアプリの共有から直接 Brake. に送れます」を `<br>` で「他のアプリの共有から」／「直接 Brake. に送れます」の2行に分割。PC版・モバイル版両方に適用。 |
| 2026-07-07 | fix(share): 「共有に追加」ボタンのテキスト中央揃え修正（encrypt.js + shared/header.js）: `.share-card-btn` / `.mob-share-btn` を `display:inline-block` → `display:flex; align-items:center; justify-content:center; width:100%; box-sizing:border-box` に変更。letter-spacing による右端余白で左寄りに見えていた問題を解消。 |
| 2026-07-07 | style(share): 共有導線カードの位置調整（encrypt.js）: 縦位置を `top:50%; transform:translateY(-50%)` でフォーム垂直中央に変更、横オフセットを `left:calc(100%+40px)` に拡大してフォームから適度に離す。 |
| 2026-07-07 | feat(lp): 「共有に追加」導線カード（PC右余白）を実装（encrypt.js）: ①`.hero-form-wrap` に `position:relative` を追加。②`.share-card`（position:absolute / left:calc(100%+24px) / top:24px / width:224px）CSS追加。`@media(min-width:880px)` で表示、スマホは非表示。背後に重なり紙2枚（`.share-paper1` #f7ddcf コーラル / `.share-paper2` #e6ebf0 青）をtranslateでずらし配置。③HTML: `</div><!-- /form-card-wrap -->` 直後に`.share-card`DOM挿入（説明文＋「共有に追加」ボタン accent-grad）。④JS: スクリプト先頭に `beforeinstallprompt` 捕捉（`_deferredInstallPrompt` グローバル）。カード初期化IIFE: standalone判定→非表示 / iOS→`IOS_SHORTCUT_URL`（現在空・後で差し込む）→空なら「準備中…」 / Android/PC→`prompt()` 発火→承認で非表示 / その他→静かに何もしない。 |
| 2026-07-07 | fix(form): カスタム時間入力の重複・ドット入力バグを修正（encrypt.js）: `input` ハンドラ内の `tvEl.value` 書き戻し2箇所（正規化・上限クリップ）を削除。`input` 中の `.value` 代入は Android Chrome 等で `input` 再発火・IME組み込み破壊・カーソルリセットを引き起こし「22→222」「.が打てない」の両バグの根本原因。正規化（全角→半角・読点→ピリオド・複数小数点除去）と上限クリップ・切り上げ丸めはすべて `blur` 時に実施するよう `blur` ハンドラを拡張（unit ごとの max 計算追加・isNaN/空文字は1にフォールバック）。`input` ハンドラはスライダー・チップ・ライブ表示の読み取り専用更新のみに絞る。 |
| 2026-07-07 | feat(pwa): ファイル共有対応（フェーズ2.5）（encrypt.js + public/）: ①`public/manifest.json` の share_target を GET → POST（enctype:multipart/form-data）に変更。`files: [{name:"file", accept:[image/* / video/* / audio/* / text/* / application/*]}]` を追加。②`public/sw.js` を全面更新: POST `/` を検知して `req.formData()` からファイル・text/url/title を取り出し `_pendingShares('latest')` に保存 → `/?share=pending` へ 303 リダイレクト。`message` ハンドラで `get-share` メッセージに応答しデータを返却。③`encrypt.js` に `applySharedFile(file)` 関数を追加（`clearFileSelection` 直後・モジュールレベル）。④`encrypt.js` に `?share=pending` 非同期 IIFE を追加: SW に MessageChannel で `get-share` → ファイルあり→`applySharedFile(files[0])` / テキスト/URLのみ→既存フェーズ1フローへ合流。`history.replaceState` でクエリ消去。 |
| 2026-07-07 | feat(pwa): PWA化・Android 共有シート対応（フェーズ2）（encrypt.js + public/）: ①`public/icon-192.png`・`public/icon-512.png` を favicon.svg から qlmanage+sips で生成。②`public/manifest.json` を新規作成（name/short_name "Brake."・start_url "/"・display "standalone"・theme_color/background_color #fdfbf5・icons 2枚・share_target GET action:"/" params:{title,text,url}）。③`public/sw.js` を新規作成（install→skipWaiting / activate→古いキャッシュ掃除+claim / fetch→network透過。キャッシュ名 brake-pwa-v1。decrypt.js の sadocrypt_cache_ には触れない）。④`encrypt.js` head に `<link rel="manifest">` 追加。⑤共有受け取り IIFE を更新: `title`/`text`/`url` の3パラメータをマージ（text+url→連結・片方のみ→そのまま・titleはフォールバック）してフェーズ1の既存分岐に流す。⑥ページ末尾に `navigator.serviceWorker.register('/sw.js')` 追加。 |
| 2026-07-07 | feat(share): URLクエリ共有受け取り口（フェーズ1）（encrypt.js）: `?text=…&t=…` を読んで3分岐。①text+t（正の整数・上限以内）: `#msg` に投入・`#tv=#t/#tu='s'` セット・`doEncrypt()` 自動実行。②text のみ（t なし/不正）: `#msg` に投入・ボタン活性・自動暗号化しない。③text なし: 通常着地。処理直後に `history.replaceState(null,'',location.pathname)` でアドレスバー・履歴から平文除去（思想上必須）。t バリデーション: `Number.isFinite(t) && t===Math.floor(t) && t>0 && t<=MAX_LOCK_S_CLI`（小数・負数・巨大値は不正扱い）。`updateRunBtn()`・URL下線更新も一緒に呼ぶ。 |
| 2026-07-07 | feat(form): 送信ボタン活性化フィードバック（encrypt.js）: `#msg` が空/空白のみのとき `.btn-run--off` クラスを付与し `background:#f7ded3/color:#e0a892/box-shadow:none/cursor:not-allowed` で非活性表示。初期表示は空なので非活性。input イベントで `value.trim().length===0` を再評価してトグル。ボタン click ハンドラで `--off` クラスがあれば即 return（doEncrypt 実行しない）。既存 `.btn-run` の `transition` に `background/color/box-shadow .55s ease` を追記（クラス付与で自然にアニメーション）。`:hover` 時の opacity 揺れを防ぐため `.btn-run--off:hover{opacity:1}` を追加。 |
| 2026-07-07 | feat(form): textarea Enter キーで単体URL→暗号化実行（encrypt.js）: `#msg` の keydown で Enter を分岐。①IME変換中（`e.isComposing` または `keyCode===229`）は即リターン（最優先）。②Shift+Enter は常に改行（return）。③タッチデバイス（`pointer:fine` でない）は常に改行（return）。④PC + 素のEnter: `value.trim()` が `isValidHttpUrl()` で単体URL と判定されたら `preventDefault()` して `doEncrypt()`、それ以外はデフォルト改行。二重発火なし（ボタン click と別経路）。 |
| 2026-07-07 | feat(form): URL検知時に入力欄下へグラデ下線フェードイン（encrypt.js）: `#msg` の入力値が http/https の有効URLと判定されたとき `.fi-inrow` 直下の `.fi-url-line` に `.visible` クラスを付与し `opacity:0→1`・`0.5s ease` でフェードイン。テキストまたはURL削除時は即消去。CSS: `.fi-url-line{height:2px;background:linear-gradient(90deg,#ef8a63,#a8bba0);border-radius:0 0 8px 8px;opacity:0;transition:opacity .5s ease}` + `prefers-reduced-motion:reduce` で `transition:none`。JS: `isValidHttpUrl(s)` で `new URL()` 検証（http/https のみ）、`contentInput` の input イベントで `classList.toggle('visible', …)`。暗号化・機能は変更なし。 |
...
| 2026-07-08 | revert(hero): 夕焼けグラデ演出を全削除・背景を静的生成りに戻す（encrypt.js）: スライダー連動の背景色変化（`updateHeroBg()`・`_BG_TOP`/`_BG_BOT`/`_ease`/`_samp` 全体）を削除。`.hero` を `background:var(--iri-grad)`（静的 170deg 生成りグラデ）に戻し `transition:background` も除去。`.hero-catch`/`.hero-sub` の `transition:color` も削除（JS 色変更なし）。コーラル底グロー（`.hero::after`）は元値 `.22/.09` で維持。背景はスライダー操作に関わらず固定、時間はスライダー位置・数字のみで伝える設計に変更。 |
| 2026-07-08 | style(hero): 夕焼けグラデ 2点修正 – transition 延長・コーラル底グロー復元（encrypt.js）: ①背景・テキスト色の transition を `0.5s linear` → `1.2s ease` に延長（スライダー切替でふわっと溶ける感触に）。②`.hero::after` で旧 `.hero-glow` 相当のコーラル底グローを復元（`rgba(240,135,106,.16)` → transparent、height:240px、z-index:0）。前回の3層一括削除で `.hero-glow`（常時コーラル暖色、初期 opacity=1）を夜専用 2層と誤って同梱削除していた問題を修正。新夕焼けグラデとの共存: `::after` 透過率を元値 `.22` から `.16` に控えめにし、夜状態（t=1, 底=茜紫）でも床面の暖色として自然に馴染む。 |
| 2026-07-08 | style(hero): 送る側フォームの背景を夕焼けグラデ演出に刷新（encrypt.js）: 旧演出（`.hero-sky`/`.hero-glow`/`.hero-night` 3層オーバーレイ）を削除し、スライダー操作に連動する暖色グラデに置き換え。TOP/BOT 5点カラーストップ（生成り→杏→コーラル→茜→茜紫）を smoothstep 二重がけ（_ease関数）+ セグメント内 ease 補間（_samp関数）でサンプリング。.hero 背景を JS 直書き（`transition:background 0.5s linear`）で更新。t=0.45 超えで `.hero-catch`/`.hero-sub` のテキスト色を白系に滑らかに遷移させ可読性を維持。スライダー・チップ・カスタム入力のいずれに連動（renderFromTruth/renderPreview の updateHeroBg() 呼び出し）。HTML 上のオーバーレイ div 3個と対応 CSS も全削除。 |
| 2026-07-08 | fix(cache): 復号完了キャッシュが常に無効化されるバグを修正（handlers.js + decrypt.js）: 根本原因は2段の連鎖バグ。①`handlers.js` の `puzzleJSON` に `target_seconds` が含まれておらず、クライアントでは `P.target_seconds=undefined` → `writeCache` が `exp:NaN` で保存していた。②`readCache` の `!data.exp` チェックが `!NaN===true` を評価し保存直後に即削除していた。修正: `handlers.js` に `target_seconds: puzzle.target_seconds || 0` を追加。`readCache` の判定を `typeof data.exp!=='number'||isNaN(data.exp)` に変更。`writeCache` に `isFinite` ガードを追加し `target_seconds` 欠落時は `cc/376223` で近似 TTL を算出。これにより復号完了後のリロードで即表示が機能する。resume 機構（RESUME_KEY）とは独立した別機構であり混線なし。 |
| 2026-07-08 | fix(resume): 復号リロード時の計算リセットバグを修正（public/decrypt-worker.js + src/pages/decrypt.js）: ①`decrypt-worker.js`: `progress` メッセージに `x:_x`（BigInt）を追加し、checkpoint 間隔を 5000ms→1000ms に短縮。②`decrypt.js`: `_latestResumeData` 変数（`{x:hex, i:decimal}`）を追加し、Worker `progress` ハンドラとフォールバックループで毎 chunk 更新。`beforeunload` イベントで `localStorage.setItem(RESUME_KEY, ...)` を同期実行し、ページアンロード直前の最新状態を確実に保存。完了時は `_latestResumeData=null` にセットし完了後の誤保存を防止。修正前は `checkpoint`（5秒ごと）と `onmessage` の非同期受信の競合により、特に最初の5秒以内のリロードで全進捗が失われていた。 |
