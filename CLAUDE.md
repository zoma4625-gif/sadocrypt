...
| 2026-07-02 | /time-lock テキスト刷新＋事実カード・締めブロック追加（`src/pages/time-lock.js`）: ①リード2段落→1段落（鉤括弧内を白・強調）。②3枚の事実カード（`.tl-fact-grid`/`.tl-fact-card`、横並び・モバイル縦積み）をリード直下に追加。③「仕組み」本文2段落に差し替え（「仕掛けは、それだけです。」白強調）。④h2「なぜ、スキップできないのか」（読点追加）・本文3段落差し替え（「順序そのものが壁」白強調）。⑤「Brake.での実装」本文3段落差し替え（「運営者にも手段がありません。」白強調）。⑥締めブロック（`.tl-close`・`.tl-close-line2`・22px/700）をmain末尾に追加、「計算量」緑(.hl)。既存フォントサイズ・weight・色・余白は変更なし。 |
