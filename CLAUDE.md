...
| 2026-07-07 | fix(form): 送信ボタン案内の常時スペース確保を廃止（encrypt.js）: `#run-hint` を `position:absolute; top:calc(100%+4px)` に変更し `.form-run-wrap` に `position:relative` を追加。押す前・消えた後はレイアウトへの影響ゼロ（隙間なし）。`min-height:1.4em` と `margin-top:8px` を削除。opacity フェードと入力後自動消去は維持。 |
