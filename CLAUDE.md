...
| 2026-07-09 | fix(encrypt): Mac Safari 画像「Load failed」を修正（encrypt.js）: 根本原因は `fetch()` に大きい JSON 文字列 body（画像 4MB → base64 5.3MB → JSON 5.5MB）を渡すと Mac Safari が `TypeError: Load failed` を投げること。`body: JSON.stringify(saveBody)` → `body: new Blob([JSON.stringify(saveBody)], {type:'application/json'})` に変更し、Safari が大容量 body で使う Blob コードパスを経由させる。`Content-Type` ヘッダーは Blob の type から自動付与されるため明示ヘッダーを削除。同時に Step1（暗号化）・Step2（送信）を別 try/catch に分離し「暗号化エラー:」「送信エラー:」プレフィックスで失敗ステップを特定可能に。アニメリセット共通処理を `_resetBridge()` に抽出。 |
