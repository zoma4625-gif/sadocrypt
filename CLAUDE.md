...
| 日付 | 変更内容 |
|------|----------|
| 2026-07-09 | feat(result): 復号完了カードに「via Brake.」導線を追加（decrypt.js）: 開封記録行の右端に `<a class="letter-via">` を静かに配置。「Brake」後のピリオドをコーラル #ef8a63、リンク本体は薄暖色 #c9997e、ホバーで #c56b47。下線なし・新タブなし（同タブ遷移）。ファイル系（画像/動画/音声/その他）は `.letter-sig`（flex space-between）ラッパーで日付行と横並び。テキスト/URL/YouTube は既存 `letter-foot-btns` の末尾に追加。全7コンテンツタイプで同一位置に出力。 |
