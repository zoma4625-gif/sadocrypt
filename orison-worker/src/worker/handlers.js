 ...
 */
export async function handleSave(request, env, lang) {
    lang = lang || 'ja';
    try {
        // ボディサイズ事前チェック（8MB超は即413）
        const contentLength = parseInt(request.headers.get('Content-Length') || '0', 10);
        if (contentLength > 8_000_000) {
            return new Response(JSON.stringify({ error: T('api.err.toolarge', lang) }), {
        ...
        // 必須フィールド存在チェック
        if (!x0 || !N || !cc || !iv || !ct) {
            return new Response(JSON.stringify({ error: T('api.err.missing', lang) }), {
        ...
        return new Response(JSON.stringify({ error: T('api.err.savefail', lang) }), {
