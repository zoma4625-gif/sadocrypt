...
<ol class="legal-ol"><li><strong>アクセスログ等</strong>　IPアドレス、ブラウザの種類・バージョン・言語設定、OS・デバイスの種類、アクセス日時、リクエストURL・HTTPメソッド・ステータスコード、参照元URL（Referer。ただし当方は <code>Referrer-Policy: strict-origin-when-cross-origin</code> を設定しているため、他サイトへの遷移時はオリジン（<code>https://brake.run</code>）のみ送信され、パスやクエリは送信されません）、その他通信に付随するメタデータ。これらは本サービスの保守・障害対応・不正利用防止・セキュリティ確保のため収集されます。</li></ol>
...
<h3 class="legal-h3">2-3. 共有機能で受け取ったデータの扱い</h3>
<p>他のアプリからの共有で受け取った内容は、暗号化の準備のためにお使いの端末のブラウザ内で一時的に扱われます。URLに含まれた内容はページ読み込み直後にアドレスバーと閲覧履歴から消去されます。共有された内容が当方のサーバーに送信されるのは、あなたが「時間の鍵をかける」を実行した時のみです。</p>
<hr class="legal-hr">
...
<ol class="legal-ol"><li><strong>ローカルストレージ</strong>　本サービスは、復号完了時に、当該パズルの復号鍵（x_final）を利用者の端末のローカルストレージに保存します。これは、同一ブラウザから同一URLに再アクセスした際に計算を省略し即座に復号を完了させるためのものです（いわゆる「キャッシュ」）。ローカルストレージに保存された情報は、利用者の端末にのみ保存され、当方のサーバーには送信されません。保存された情報は有効期限（当該コンテンツの設定時間＋30日）の経過後、または復号に失敗した場合に自動削除されます。利用者はブラウザの設定からローカルストレージを手動で削除することができます。</li></ol>
<ol class="legal-ol"><li><strong>復号再開データ</strong>（<code>brake_resume_[ID]</code>）　復号計算の途中でページを離れた場合に計算を再開できるよう、計算中の値とループ回数をローカルストレージに保存します。メッセージやファイルの中身そのものは含まれません。復号計算中に約1秒ごと、およびページを離れる直前に保存され、復号が完了した時点で自動削除されます。ローカルストレージに保存された情報は、利用者の端末にのみ保存され、当方のサーバーには送信されません。利用者はブラウザの設定からローカルストレージを手動で削除することができます。</li></ol>
